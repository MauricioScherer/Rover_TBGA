...
{
    public PetriNet rocket;       // Rede de Petri do Lander
    ...
    void Start()
    {
        ...
        rocket = new PetriNet("Assets/rocket.pflow");    //instancia RdP do Lander e constroi sua rede...
        
        StartCoroutine("executaTimer");
    }


    /*
     * Exemplo de criacao de timer...
     */

    IEnumerator executaTimer()
    {
        for(int i=0; i< 20; i++) 
        {
    ...
    void Update()
    {
        rocket.ExecCycle();  // executa um ciclo da RdP do Lander
        ...
        {
            canDeployFeet = true;
            rocket.GetPlaceByLabel("PodePousar").Tokens = 1;
        }
    ...
    void OnCollisionEnter2D(Collision2D hitInfo)
    {
        ...
        {
                rocket.GetPlaceByLabel("LanderDestruido").Tokens = 1;
                HandleLanderDestroy();
        }
        else if (hitInfo.relativeVelocity.magnitude > 1)
        {
            rocket.GetPlaceByLabel("LanderPousado").Tokens = 1;
                HandleLanderDestroy();
        }
    }
    ...
    /*
     * Metodos PRNG: serao empregados para o prototipo a partir do GB
     * para temporizacao de eventos
     *    
     */

    public float geraExp()
    {
        return -4 * Mathf.Log(1f - 0.3f);
    }
}
